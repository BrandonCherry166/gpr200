...
unsigned int loadTexture2D(const char* filePath, int filterMode, int wrapMode, bool flipIt)
{
	...
	if (nrChannels == 3)
	{
		if (data)
		{
			glTexImage2D(GL_TEXTURE_2D, 0, nrChannels, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
	...
	else if(nrChannels == 4)
	{
		if (data)
		{
			glTexImage2D(GL_TEXTURE_2D, 0, nrChannels, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
			glGenerateMipmap(GL_TEXTURE_2D);
		}
		else
		{
